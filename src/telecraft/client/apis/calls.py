...
class CallsAPI:
    def __init__(self, raw: Any, *, enabled: bool = False) -> None:
        self._raw = raw
        self._manager = CallsManager(raw=raw, enabled=enabled)
    ...
    async def call(self, peer: PeerRef, *, timeout: float = 20.0) -> CallSession:
        return await self._manager.call(peer, timeout=timeout)
    ...
    async def send_signaling_data(
        self,
        call_ref: PhoneCallRef,
        data: bytes,
        *,
        timeout: float = 20.0,
    ) -> Any:
        return await self._signaling.send_signaling_data(call_ref, data, timeout=timeout)
    ...
    async def confirm(
        ...
        *,
        ...
        return await self._signaling.confirm_call(
            call_ref,
